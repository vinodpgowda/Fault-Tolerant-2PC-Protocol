    ...
    while True:
        try:
            ...
            if message.startswith("Decision:") or message.startswith(
                ...
                if not auto_abort:
                    # Prompt for commit/abort for Part 3
                    while True:
                        response = input(
                            f"Coordinator's message: '{received_message}'. Reply with 'Commit' or 'Abort': "
                        )
                        if response in ["Commit", "Abort"]:
                            break
                        print("Invalid input. Please enter either 'Commit' or 'Abort'.")
                else:
                    response = "Abort"
...
def start_participant(participant_index):
    ...
    participant_socket.settimeout(30)  # Set a timeout for waiting for messages
